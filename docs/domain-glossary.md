...
| **Goal** | A meaningful change or achievement pursued within one season. It states what matters at a strategic level. | A daily task, activity log, or automatically measurable result. | Belongs to a season; may have outcomes, milestones, habits, recurring actions, projects, and tasks. | Initial |
| **Outcome** | A result or condition used to determine whether a goal progressed or succeeded. It makes success observable. | A task, habit, milestone, or action performed merely as effort. | Belongs to a goal; may be binary, numeric, percentage-based, or descriptive. Scoring is undecided. | Initial |
| **Milestone** | A significant intermediate checkpoint that supports one goal. It makes a larger goal easier to navigate. | A recurring behavior or proof that the final outcome was achieved. | May give rise to daily tasks; supports one goal. | Initial, limited |
